...
caminho_vendas_ouro = os.path.join(diretorio_base, "vendas_consolidadas_ouro.parquet")

# Inicializando a sessão na AWS
...
def upload_to_s3_otimizado(file_path, bucket, object_name):
    try:
        ...
        print(f"✅ Atualização concluída com sucesso: s3://{bucket}/{object_name}\n")
...
# ==========================================
print("Iniciando pipeline de Upload AWS (Camada Ouro - Modo Substituição)...\n")

# 1. Verifica o bucket
criar_bucket(nome_bucket=bucket_name, regiao=regiao_aws)

# 2. Faz o upload substituindo o arquivo anterior
upload_to_s3_otimizado(
    file_path=caminho_vendas_ouro, 
    bucket=bucket_name, 
    object_name='ouro/vendas/vendas_consolidadas_ouro.parquet'
)